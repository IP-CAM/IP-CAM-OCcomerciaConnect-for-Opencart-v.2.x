...
class Twig
{
    ...
    private function handleMethod($class, $method)
    {
        ...
        //should use
        $this->twig->addFunction("comercia_" . $funcName, new Twig_SimpleFunction("comercia_" . $funcName,
            function () use ($class, $method) {
                return call_user_func_array([Util::$class(), $method], func_get_args());
            }
        ));
        //for compatibility
        $this->twig->addFunction("util_" . $funcName, new Twig_SimpleFunction("util_" . $funcName,
            function () use ($class, $method) {
                return call_user_func_array([Util::$class(), $method], func_get_args());
            }
        ));
